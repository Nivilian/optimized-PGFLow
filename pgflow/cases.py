...
# from solver.solver.a_star_maze import Maze as asMaze
...
class Case:
    ...
    def __init__(self, name: str, mazeWidth = [-4,4], mazeHeight = [-4,4], tether_mode = 0):
...
class Cases:
    def __init__(self, cases: dict, case_name: str = "default", mazeWidth = [-4,4], mazeHeight = [-4,4], tether_mode = 0) -> None:
    ...
    @classmethod
    def get_case(cls, file_name: str, case_name: str, mazeWidth = [-4,4], mazeHeight = [-4,4], tether_mode = 0) -> Case:
        ...
        cases_instance.case_setup(case_name, mazeWidth, mazeHeight, tether_mode)  # Setup the specific case
        # return the case
    ...
    def case_setup(self, case_name: str, mazeWidth = [-4,4], mazeHeight = [-4,4], tether_mode = 0) -> None:
        self.case_name = case_name

#Yuan_edit_below: 
        self.case = Case(self.case_name, tether_mode=tether_mode, mazeWidth=mazeWidth, mazeHeight=mazeHeight)
#Yuan_edit_above^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#Yuan_edit_below: 
        ...
        if self.tether_mode == 1:
            # self.obstacles = [([list(building.shapelyPoly.coords) in building]) for building in self.case.buildings]
            self.obstacles = [building["vertices"] for building in buildings]
            # for building in self.case.buildings:
            #     print("direct_vertices:",building.vertices)
            #     print("building coord:",building.shapelyPoly)
            # print("self.obstacles: ",self.obstacles[5])
            self.mazeCPL = asMaze(self.obstacles, maze_width=self.mazeWidth, maze_height=self.mazeHeight)
            self.concave_record = self.mazeCPL.concave_record
            self.graph = self.mazeCPL.basic_graph
            self.theory_map = self.mazeCPL.basic_theory_map
        else:
            self.obstacles = None
            self.mazeCPL = None
            self.concave_record = None
            self.graph = None
            self.theory_map = None
#Yuan_edit_above^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

        vehicles = self.cases[self.case_name]["vehicles"]
        self.case.vehicle_list = self.obtain_vehicles(vehicles, self.obstacles, self.graph, self.concave_record, self.theory_map,self.mazeWidth, self.mazeHeight, self.tether_mode)
    ...
    @classmethod
    def obtain_vehicles(cls, vehicle_data: list[dict], 
    obstaclesInput = None, 
    graphInput = None, 
    concave_recordInput = None, 
    theory_mapInput = None,
    maze_widthInput = [-4,4],
    maze_heightInput = [-4,4],
    tether_modeInput = 0) -> list[Vehicle]:
...
#Yuan_edit_below: 
                ...
                graph = graphInput,
                concave_record=concave_recordInput,
                theory_map=theory_mapInput,
                maze_width=maze_widthInput,
                maze_height=maze_heightInput,
                tether_mode=tether_modeInput,
#Yuan_edit_above^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
            )
            myVehicle.ID = ID
