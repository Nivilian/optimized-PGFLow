...
# filename = 'test8x20_5d.json' #case name from scenebuilder
filename = 'test8x20_5d_ts.json'
ArenaMap.size = 0.1
ArenaMap.inflation_radius = 0.2
case = Cases.get_case(filename, 'scenebuilder',mazeWidth=[-4,4], mazeHeight=[-10,10], tether_mode=1)
...
set_new_attribute(case, 'source_strength', 1)
target_speed = 0.8
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=120,
        type=int,
        help="Simulation frequency in Hz (default: 48)",
        metavar="",
    )
    parser.add_argument(
        "--control_freq_hz",
    ...
    parser.add_argument(
        "--duration_sec",
#Yuan_edit_below:   only change it from 20 -> xxx.
        default=100,
#Yuan_edit_above^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
#Yuan_edit_below: 
    camera_distance = 5  # Distance from the target point
    camera_yaw = 0  # Rotation around the vertical axis
    camera_pitch = -89  # Rotation around the horizontal axis
    camera_target_position = [0, 0, 6]  # Target position
